...
whitgl_debug_menu whitgl_debug_menu_add_float(whitgl_debug_menu debug, const char* name, const float* val)
{
...
void whitgl_debug_menu_draw(whitgl_debug_menu debug, whitgl_sprite text_sprite, whitgl_ivec draw_pos)
{
	...
	for(i=0; i<debug.num_entries; i++)
	{
		...
		switch(debug.entries[i].type)
		{
			...
			{
     			snprintf(buffer, sizeof(buffer), "%s %.2f", debug.entries[i].name, *debug.entries[i].float_val);
				break;
			}
		}
		whitgl_iaabb backing;
		backing.a.x = draw_pos.x-1;
		backing.a.y = draw_pos.y-1;
